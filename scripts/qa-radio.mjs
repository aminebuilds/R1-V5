...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(() => window.__r1?.dataManager, { timeout: 60_000 });
    await page.waitForFunction(() => window.__r1?.styleManager?._dataManager?.layers?.has('radio'), { timeout: 60_000 });
    await page.waitForFunction(
      () => document.getElementById('loading-screen')?.classList.contains('hidden'),
      { timeout: 60_000 },
    );
    await page.waitForFunction(() => (
      typeof window.__r1QaRegisterLayer === 'function'
      && typeof window.__r1QaUnregisterLayer === 'function'
    ));
    await page.evaluate(() => {
      window.__r1.viewer.camera.cancelFlight();
      window.__r1.styleManager.setPanelCollapsed('pp-toggles', true);
      window.__r1.styleManager.setPanelCollapsed('cctv-panel', true);
      window.__r1.styleManager.setPanelCollapsed('radio-panel', true);
      window.__r1.styleManager.setPanelCollapsed('global-context-panel', true);
    });
    ...
    const initialDisclosure = await page.evaluate(() => {
      ...
      const enabledBefore = window.__r1.dataManager.isEnabled('radio');
      button.click();
      return {
        enabledBefore,
        enabledAfterDisclosure: window.__r1.dataManager.isEnabled('radio'),
    ...
    await page.waitForFunction(() => {
      const state = window.__r1.dataManager.layers.get('radio').module.getUIState();
      return state.enabled && !state.loading && state.stationCount === 750;
    });
    const enableElapsedMs = Date.now() - enableStartedAt;
    const initial = await page.evaluate(() => {
      const module = window.__r1.dataManager.layers.get('radio').module;
      ...
      return {
        ...
        tunerCount: window.__r1.styleManager._radioTunerStations.length,
    ...
    const uncertainPresentation = await page.evaluate(async () => {
      const r1 = window.__r1;
      const style = r1.styleManager;
      const entry = r1.dataManager.layers.get('radio');
      ...
      r1.dataManager._refreshTogglePanel();
      ...
      const result = {
        ...
        markerSourceVisible: r1.viewer.dataSources.getByName('Radio stations')[0]?.show,
    ...
    await page.evaluate(() => {
      const r1 = window.__r1;
      const module = r1.dataManager.layers.get('radio').module;
      ...
      window.__qaRadioDisablePromise = r1.dataManager.setEnabled('radio', false, { origin: 'qa-setup' });
    });
    await page.waitForFunction(() => window.__qaRadioDisableStarted === true);
    const disablingPresentation = await page.evaluate(() => ({
      manager: window.__r1.dataManager.getLayerLifecycleState('radio'),
      presentationActive: window.__r1.dataManager.layers.get('radio').module.getUIState().presentationActive,
      markerSourceVisible: window.__r1.viewer.dataSources.getByName('Radio stations')[0]?.show,
    ...
    const explicitRevealBefore = await page.evaluate(async () => {
      const r1 = window.__r1;
      r1.styleManager.setPanelCollapsed('global-context-panel', false);
      r1.styleManager.setPanelCollapsed('radio-panel', false);
      await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
      ...
      const camera = r1.viewer.camera.positionWC;
      return {
    ...
    await page.waitForFunction(() => (
      window.__r1.dataManager.getLayerLifecycleState('radio')?.lifecycleState === 'enabling'
    ));
    ...
    await page.waitForFunction(() => window.__r1.dataManager.isEnabled('radio'));
    ...
    const explicitRevealAfter = await page.evaluate(() => {
      ...
      return {
        ...
        plays: window.__qaRadioPlayCalls.length,
        audioState: r1.dataManager.layers.get('radio').module.getUIState().audioState,
      };
    });
    ...
    for (const spec of singletonViewSpecs) {
      ...
      for (const view of [
        ...
        await page.evaluate(({ lon, height }) => {
          const viewer = window.__r1.viewer;
    ...
    const userFacingFailures = await page.evaluate(async () => {
      const r1 = window.__r1;
      const styleManager = r1.styleManager;
      const dataManager = r1.dataManager;
      const unhandled = [];
      ...
      try {
        ...
        for (const outcome of ['false', 'reject']) {
          ...
          window.__r1QaRegisterLayer(dataManager, qaLayer);
          try {
            ...
            styleManager._showToast = originalShowToast;
            await window.__r1QaUnregisterLayer(dataManager, layerId);
          }
        }
        ...
        window.__r1QaRegisterLayer(dataManager, {
          id: unrelatedLayerId,
        ...
        await pendingAction;
        styleManager._showToast = originalShowToast;
        await window.__r1QaUnregisterLayer(dataManager, unrelatedLayerId);
    ...
    await page.evaluate(async () => {
      await window.__r1.dataManager.layers.get('radio').module.update();
    });
    await page.select('#radio-filter', 'all');
    const allPalette = await page.evaluate(() => {
      const source = Array.from({ length: window.__r1.viewer.dataSources.length }, (_, index) => (
        window.__r1.viewer.dataSources.get(index)
      )).find((item) => item.name === 'Radio stations');
    ...
    const multiExpandedPanelLayout = await page.evaluate(async () => {
      const manager = window.__r1.styleManager;
    ...
    const cockpitCompactControls = await page.evaluate(async () => {
      ...
      window.__r1.styleManager._syncLeftPanelAdaptiveLayout();
      window.__r1.styleManager.cockpitView.syncSignalLayout();
      ...
      const toggleBefore = toggleLayerId
        ? window.__r1.dataManager.isEnabled(toggleLayerId) : null;
      toggleButton?.click();
      await waitForLayout();
      ...
      const toggleRestored = toggleLayerId
        ? window.__r1.dataManager.isEnabled(toggleLayerId) === toggleBefore : false;
    ...
    const firstPlayPrecondition = await page.evaluate(() => (
      window.__r1.dataManager.layers.get('radio').module.getUIState().selected?.id || null
    ));
    ...
    const postMicroDragPlayback = await page.evaluate(async () => {
      const mod = window.__r1.dataManager.layers.get('radio').module;
    ...
    const newsClusterTarget = await page.evaluate(() => {
      const r1 = window.__r1;
      const source = Array.from({ length: r1.viewer.dataSources.length }, (_, index) => r1.viewer.dataSources.get(index))
        .find((item) => item.name === 'Radio stations');
      const points = source.clustering._clusterPointCollection;
      for (let index = 0; index < (points?.length || 0); index += 1) {
        const point = points.get(index);
        if (!point.show || !Array.isArray(point.id) || !point.id.length) continue;
        const canvasPoint = r1.viewer.scene.cartesianToCanvasCoordinates(point.position);
    ...
    const newsClusterClick = await page.evaluate(() => {
      const r1 = window.__r1;
      const state = r1.dataManager.layers.get('radio').module.getUIState();
      r1.dataManager.layers.get('radio').module.stopPlayback();
    ...
    const compactVolume = await page.evaluate(() => ({
      volume: window.__r1.dataManager.layers.get('radio').module.getUIState().volume,
    ...
    const voicePause = await page.evaluate(async () => {
      ...
      await window.__r1.dataManager.layers.get('radio').module.play();
      const startedAt = performance.now();
      window.__r1.voiceCommands.setStatus('connecting', 'QA voice connect');
      const state = window.__r1.dataManager.layers.get('radio').module.getUIState();
      return {
        ...state,
    ...
    const idleVoice = await page.evaluate(() => {
      ...
      return window.__r1.dataManager.layers.get('radio').module.getUIState();
    });
    check(
    ...
    if (manualPlayTarget.visible && manualPlayTarget.enabled) {
      ...
      await page.waitForFunction(() => (
        window.__r1.dataManager.layers.get('radio').module.getUIState().audioState === 'playing'
        && window.__r1.voiceCommands.status === 'idle'
      ));
    }
    const manualTakeover = await page.evaluate(() => ({
      audioState: window.__r1.dataManager.layers.get('radio').module.getUIState().audioState,
      voiceStatus: window.__r1.voiceCommands.status,
      voiceDucked: window.__r1.dataManager.layers.get('radio').module.getUIState().voiceDucked,
    }));
    check(
    ...
    await page.evaluate(() => window.__r1.voiceCommands.setStatus('listening', 'QA standby'));
    ...
    await page.evaluate(() => window.__r1.voiceCommands.setStatus('idle', 'QA complete'));
    await page.evaluate(async () => {
      const manager = window.__r1.styleManager;
    ...
    const absoluteTuner = await page.evaluate(async () => {
      const r1 = window.__r1;
      const style = r1.styleManager;
      const radio = r1.dataManager.layers.get('radio').module;
      const slider = document.getElementById('radio-tuner-slider');
      ...
      const snapshot = () => {
        const markerId = r1.viewer.entities.values
    ...
    const tunerCancelRestore = await page.evaluate(() => {
      ...
      const previewBeforeCancel = r1.viewer.entities.values
        .find((entity) => String(entity.id).startsWith('radio:selected:'))?.id || null;
      ...
      const selectedEntity = () => r1.viewer.entities.values
    ...
    const tunerRefreshTarget = await page.evaluate(() => {
      const r1 = window.__r1;
      const slider = document.getElementById('radio-tuner-slider');
      const stations = r1.styleManager._radioTunerStations;
      const targetIndex = 1;
      ...
      return {
        ...
        clientX,
        clientY: rect.top + rect.height / 2,
        generation: r1.dataManager.layers.get('radio').module.getUIState().tuningCatalogGeneration,
        plays: window.__qaRadioPlayCalls.length,
      };
    });
    ...
    const tunerRefreshRelease = await page.evaluate((target) => {
      ...
      const frozenGeneration = r1.dataManager.layers.get('radio').module.getUIState().tuningCatalogGeneration;
    ...
    const tunerDirectRelease = await page.evaluate(async () => {
      ...
      const targetId = r1.styleManager._radioTunerStations[targetIndex]?.id || null;
      const rect = slider.getBoundingClientRect();
    ...
    const tunerCommitTarget = await page.evaluate(() => {
      ...
      const station = r1.styleManager._radioTunerStations[targetIndex];
      const rect = slider.getBoundingClientRect();
      ...
      return {
        id: station.id,
        targetIndex,
        count: r1.styleManager._radioTunerStations.length,
        frozenSignature: r1.styleManager._radioTunerBandSignature,
        startX: rect.left + 7 + (rect.width - 14) * currentRatio,
        targetX: rect.left + 7 + (rect.width - 14) * targetIndex
          / Math.max(1, r1.styleManager._radioTunerStations.length - 1),
    ...
    const tunerCommit = await page.evaluate((target) => {
      ...
      return {
        ...
        signatureStable: r1.styleManager._radioTunerBandSignature === target.frozenSignature,
        selectedIndex: r1.styleManager._radioTunerStations.findIndex((item) => item.id === state.selected?.id),
        sliderSlot: Number(slider.value),
        ratio: Number(document.getElementById('radio-tuner').style.getPropertyValue('--radio-tuner-ratio')),
        pinned: r1.styleManager._radioTunerBandPinnedForNavigation,
      };
    }, tunerCommitTarget);
    check(
    ...
    const microDragStart = await page.evaluate(() => {
      const style = window.__r1.styleManager;
      const slider = document.getElementById('radio-tuner-slider');
      const rect = slider.getBoundingClientRect();
    ...
    const viewportTuner = await page.evaluate(async (selectedId) => {
      ...
      const current = r1.viewer.camera.positionCartographic;
      const oppositeLongitude = ((current.longitude + 2 * Math.PI) % (2 * Math.PI)) - Math.PI;
      r1.viewer.canvas.dispatchEvent(new WheelEvent('wheel', { bubbles: true, deltaY: 1 }));
      r1.viewer.camera.setView({
        destination: r1.viewer.scene.globe.ellipsoid.cartographicToCartesian({
      ...
      r1.viewer.camera.changed.raiseEvent();
    ...
    const nextNeedleBefore = await page.evaluate(() => {
      const style = window.__r1.styleManager;
      const selectedId = window.__r1.dataManager.layers.get('radio').module.getUIState().selected?.id;
    ...
    const filteredTunerCenter = await page.evaluate(() => {
      ...
      const preview = {
        ...
        slot: Number(slider.value),
        ratio: Number(document.getElementById('radio-tuner').style.getPropertyValue('--radio-tuner-ratio')),
        markerId: r1.viewer.entities.values
          .find((entity) => String(entity.id).startsWith('radio:selected:'))?.id || null,
    ...
    const fullPoolNavigation = await page.evaluate(() => {
      const r1 = window.__r1;
      const radio = r1.dataManager.layers.get('radio').module;
      const pool = r1.styleManager._radioTunerPool;
    ...
    const failedTuner = await page.evaluate(async () => {
      ...
      const slider = document.getElementById('radio-tuner-slider');
      const rect = slider.getBoundingClientRect();
      const count = r1.styleManager._radioTunerStations.length;
      ...
      const failed = r1.dataManager.layers.get('radio').module.getUIState();
      r1.dataManager.layers.get('radio').module.stopPlayback();
      const stopped = r1.dataManager.layers.get('radio').module.getUIState();
      return {
        failed: {
    ...
    if (markerTarget?.found) {
      ...
      await page.waitForFunction(
        (id) => window.__r1.dataManager.layers.get('radio').module.getUIState().selected?.id === id,
        {},
        markerTarget.id,
      );
      const offsetSelected = await page.evaluate(() => window.__r1.dataManager.layers.get('radio').module.getUIState().selected?.id);
    ...
    await page.evaluate(() => {
      ...
      radio.stopPlayback();
      Array.from({ length: r1.viewer.dataSources.length }, (_, index) => r1.viewer.dataSources.get(index))
      ...
      r1.dataManager.layers.get('flights').enabled = true;
    });
    await sleep(500);
    await page.evaluate(() => window.__r1.styleManager.setPanelCollapsed('global-context-panel', true));
    await page.evaluate(() => {
      const button = document.getElementById('context-radio-toggle-btn');
      if (button.getAttribute('aria-expanded') !== 'true') button.click();
    });
    ...
    await page.evaluate(() => {
      window.__qaRadioCameraHeight = window.__r1.viewer.camera.positionCartographic.height;
    });
    ...
    await page.waitForFunction(() => window.__r1.dataManager.layers.get('radio').module.getUIState().audioState === 'playing');
    const playing = await page.evaluate(async () => {
      const radio = window.__r1.dataManager.layers.get('radio').module;
      const viewer = window.__r1.viewer;
      const selectedEntity = window.__r1.viewer.entities.values
      ...
      regularMarkerSize: Array.from({ length: window.__r1.viewer.dataSources.length }, (_, index) => window.__r1.viewer.dataSources.get(index))
    ...
    const trackedRadioControls = await page.evaluate(async () => {
      ...
      const run = async (ownerPrefix, buttonId) => {
        ...
        owner.r1TrackedId = `${ownerPrefix}:qa-radio-camera-owner`;
        ...
        const result = {
          ...
          trackedId: viewer.trackedEntity?.r1TrackedId || null,
        };
    ...
    const delayedTrackingOwner = await page.evaluate(() => {
      ...
      owner.r1TrackedId = 'flights:qa-delayed-radio-fallback-owner';
      viewer.trackedEntity = owner;
      window.__qaDelayedRadioTrackingOwner = owner;
      window.__qaRejectRadioPlay();
      return owner.r1TrackedId;
    });
    await page.waitForFunction(() => window.__r1.dataManager.layers.get('radio').module.getUIState().audioState === 'playing');
    const delayedFallbackOwnership = await page.evaluate((beforeFlyToCalls) => {
      ...
      const result = {
        ...
        trackedIdentityPreserved: viewer.trackedEntity === owner,
        trackedId: viewer.trackedEntity?.r1TrackedId || null,
        selectedId: window.__r1.dataManager.layers.get('radio').module.getUIState().selected?.id || null,
      };
      viewer.trackedEntity = undefined;
      viewer.entities.remove(owner);
    ...
    await page.evaluate(async () => {
      const viewer = window.__r1.viewer;
      window.__r1.styleManager.toggleCleanView(true);
      viewer.resize();
      viewer.scene.requestRender();
      await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
    });
    ...
    await page.evaluate(() => {
      const viewer = window.__r1.viewer;
      window.__r1.styleManager.toggleCleanView(false);
      viewer.resize();
      viewer.scene.requestRender();
    });
    await page.evaluate(() => {
    ...
    await page.evaluate(() => {
      window.__qaRadioFlyToCalls = [];
      window.__r1.dataManager.layers.get('flights').enabled = false;
    });
    ...
    const radioOnlyFocusCalls = await page.evaluate(() => {
      ...
      const heights = window.__qaRadioFlyToCalls.map((options) => (
        window.__r1.viewer.scene.globe.ellipsoid.cartesianToCartographic(options.destination).height
      ));
      window.__r1.viewer.camera.flyTo = window.__qaRadioOriginalFlyTo;
    ...
    const stagedGlobeNavigation = await page.evaluate(async () => {
      ...
      owner.r1TrackedId = 'flights:qa-radio-recenter-stage-owner';
      viewer.trackedEntity = owner;
      completeFlight(0);
    ...
    await page.evaluate(() => {
      window.__r1.styleManager.setPanelCollapsed('radio-panel', true);
      window.__r1.styleManager.setPanelCollapsed('global-context-panel', false);
    });
    const companion = await page.evaluate(() => ({
      radioEnabled: window.__r1.dataManager.isEnabled('radio'),
      state: window.__r1.dataManager.layers.get('radio').module.getUIState().audioState,
      radioCollapsed: document.getElementById('radio-panel').classList.contains('collapsed'),
      contextCollapsed: document.getElementById('global-context-panel').classList.contains('collapsed'),
    ...
    const expandedContextRadioAfter = await page.evaluate(() => {
      const r1 = window.__r1;
      r1.styleManager._renderRadioState(r1.dataManager.layers.get('radio').module.getUIState());
      const launcher = document.getElementById('context-radio-toggle-btn');
      const radio = document.getElementById('radio-panel');
      const scroller = document.querySelector('#global-context-panel .global-context-panel-inner');
      const camera = r1.viewer.camera.positionWC;
      return {
        ...
        camera: { x: camera.x, y: camera.y, z: camera.z },
        plays: window.__qaRadioPlayCalls.length,
        state: r1.dataManager.layers.get('radio').module.getUIState().audioState,
      };
    });
    ...
    const filtered = await page.evaluate(() => window.__r1.dataManager.layers.get('radio').module.getUIState());
    ...
    await page.evaluate(async () => {
      const r1 = window.__r1;
      await r1.dataManager.setEnabled('radio', false);
      await r1.dataManager.setEnabled('radio', true);
    });
    const restored = await page.evaluate(() => ({
      state: window.__r1.dataManager.layers.get('radio').module.getUIState(),
      plays: window.__qaRadioPlayCalls.length,
      radioCollapsed: document.getElementById('radio-panel').classList.contains('collapsed'),
    }));
    check(
    ...
    await page.evaluate(() => window.__r1.styleManager.setPanelCollapsed('radio-panel', false));
    await sleep(350);
