...
try {
  ...
  await page.waitForFunction(() => window.__r1?.styleManager, { timeout: 60_000 });
  ...
  const contactMissionHandoff = await page.evaluate(async () => {
    const { styleManager, dataManager } = window.__r1;
  ...
  const cancelledMissionEntry = await page.evaluate(async () => {
    const styleManager = window.__r1.styleManager;
    const dataManager = window.__r1.dataManager;
    ...
    window.__r1QaRegisterLayer(dataManager, siblingModule);
    ...
    try {
      ...
      await window.__r1QaUnregisterLayer(dataManager, siblingId);
    }
  });
  check(
  ...
  const voiceMissionEntry = await page.evaluate(async () => {
    const { dataManager, styleManager } = window.__r1;
  ...
  await page.evaluate(() => window.__r1.dataManager.setEnabled(
    'flights',
    true,
    { origin: 'user' },
  ));
  await page.waitForFunction(() => {
    const layer = window.__r1?.dataManager?.layers?.get('flights')?.module;
  ...
  await page.waitForFunction(
    () => Boolean(window.__r1.viewer.trackedEntity?.position),
    { timeout: 10_000 },
  );
  const preselectedContactAdoption = await page.evaluate(async () => {
    const { styleManager, dataManager, viewer } = window.__r1;
    ...
    try {
      ...
      return {
        ...
        viewerTrackedId: viewer.trackedEntity?.r1TrackedId || null,
      ...
      if (!deferCleanup && dataManager.layers.has(blockerId)) {
        ...
        await window.__r1QaUnregisterLayer(dataManager, blockerId);
        styleManager.cockpitView.syncEntry();
      }
    }
  });
  ...
  const zoomedOutContactRefocus = await page.evaluate(async () => {
    const { dataManager, viewer } = window.__r1;
    const awareness = dataManager.layers.get('military-awareness')?.module;
    const before = awareness?.getContextSnapshot?.()?.subject || null;
  ...
  await page.waitForFunction(
    () => document.body.classList.contains('cockpit-mode')
      && window.__r1.styleManager.cockpitView.active
  ...
  const firstCockpitContact = await page.evaluate(() => {
    const awareness = window.__r1.dataManager.layers
  ...
  const contactsDetection = await page.evaluate(async () => {
    const { styleManager } = window.__r1;
    ...
    const cleanupUnregistered = await window.__r1QaUnregisterLayer(
      window.__r1.dataManager,
      cleanupBlockerId,
    );
    const cleanupBlockerAbsent = !window.__r1.dataManager.layers.has(cleanupBlockerId);
  ...
  const densityNavigation = await page.evaluate(async () => {
    ...
    const snapshot = (step) => {
      ...
      return {
        ...
        tracked: tracker?.r1TrackedId || null,
  ...
  await page.evaluate(() => window.__r1.styleManager.cockpitView.setVisionMode('noir'));
  await page.screenshot({ path: path.join(shotsDir, 'vision-noir.png') });
  await page.evaluate(() => window.__r1.styleManager.cockpitView.setVisionMode('optical'));
  ...
  const signalTransition = await page.evaluate(() => {
    ...
    window.addEventListener('r1:cockpit-signal-expanded', onExpanded);
    try {
      ...
      window.removeEventListener('r1:cockpit-signal-expanded', onExpanded);
      cockpit.signalUserCollapsed = false;
      cockpit.setSignalCollapsed(false);
    }
  });
  ...
  const contextTransition = await page.evaluate(() => {
    const cockpit = window.__r1.styleManager.cockpitView;
    ...
    window.addEventListener('r1:cockpit-context-expanded', onExpanded);
    try {
      ...
      window.removeEventListener('r1:cockpit-context-expanded', onExpanded);
  ...
  const resetResult = await page.evaluate(() => {
    const manager = window.__r1.styleManager;
    const awareness = window.__r1.dataManager.layers.get('military-awareness')?.module;
  ...
  const resetState = await page.evaluate(() => {
    ...
    const awareness = r1.dataManager.layers.get('military-awareness')?.module;
    const subjectId = awareness?.getContextSnapshot?.()?.subject?.id || null;
    const height = r1.viewer.camera.positionCartographic?.height;
    r1.styleManager.resetToGlobeView = qa.original;
    delete window.__qaCockpitReset;
    return {
      calls: qa.calls,
      cockpitActive: r1.styleManager.cockpitView.active,
      trackedEntity: Boolean(r1.viewer.trackedEntity),
