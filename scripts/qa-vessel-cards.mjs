...
export function buildVesselCardManifest({
  ...
  return {
    ...
    source: dataMode === 'live' ? 'AISStream live feed' : 'R1 synthetic AIS fixture',
...
async function main() {
  ...
  try {
    for (const key of PORT_KEYS) {
      ...
      await page.waitForFunction(() => !!window.__r1?.viewer, { timeout: 60000 });
      ...
      const dataEvidence = await page.evaluate(async ({ p, dataMode, fixtureRows }) => {
        ...
        await r1.dataManager.setEnabled('ais-live-vessels', true);
        if (dataMode !== 'synthetic') return { mode: 'live', injected: 0 };
        const seam = r1.dataManager.layers.get('ais-live-vessels')?.module?.__focusEvidence;
      ...
      const settled = await page
        .waitForFunction(() => {
          const r1 = window.__r1;
          const t = r1.tileset;
          const ais = r1.dataManager.getAll().find((l) => l.id === 'ais-live-vessels');
      ...
      const resettled = settled ? true : await page
        .waitForFunction(() => {
          const r1 = window.__r1;
          const ais = r1.dataManager.getAll().find((l) => l.id === 'ais-live-vessels');
          return r1.tileset?.tilesLoaded && (ais?.stats?.count ?? 0) > 0;
