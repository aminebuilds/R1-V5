...
try {
  ...
  await page.evaluate(async () => {
    ...
    for (const [id, entry] of r1.dataManager.layers) {
      if (entry.enabled) { try { await r1.dataManager.setEnabled(id, false, { origin: 'user' }); } catch { /* probe */ } }
    }
  });
  ...
  const measure = async (label, enabled) => {
    await page.evaluate(async (id, on) => {
      const r1 = window.__r1;
      await r1.dataManager.setEnabled(id, on, { origin: 'user' });
    }, LAYER_ID, enabled);
