 ...
 *    Drives window.__r1VoiceCommands.runner(toolName, args) in a headless
 *    page against the dev server and asserts world state via __r1 /
 *    __r1Annotations (camera altitude bands, annotation counts, route
...
async function runRoutingLayer() {
  ...
  const logDir = path.join(ROOT, '.r1-logs', 'qa-voice-routing');
...
async function runBehaviorLayer() {
  ...
  try {
    ...
    await page.waitForFunction(
      () => window.__r1?.viewer && window.__r1VoiceCommands?.runner && window.__r1Annotations,
      { timeout: 120000, polling: 250 },
    );
    // House rule: the intro flight clobbers teleports issued mid-flight.
    await page.evaluate(() => window.__r1.viewer.camera.cancelFlight());

    const run = (name, args) => page.evaluate(
      (n, a) => Promise.resolve(window.__r1VoiceCommands.runner(n, a))
        .catch((e) => ({ ok: false, error: String(e?.message || e), _threw: true })),
      name, args,
    );
    const camState = () => page.evaluate(() => {
      const c = window.__r1.viewer.camera;
    ...
    await settle(2500);
    let annoCount = await page.evaluate(() => window.__r1Annotations?.list?.().length ?? window.__r1Annotations?.count?.() ?? -1);
    ...
    const ownerTransfer = await page.evaluate(async () => {
      const app = window.__r1;
      const runner = window.__r1VoiceCommands?.runner;
    ...
    await page.waitForFunction(
      () => window.__r1.viewer.camera.positionCartographic.height / 1000 > 12000,
    ...
    await page.waitForFunction(
      () => window.__r1.viewer.camera.positionCartographic.height / 1000 < 900,
    ...
    for (let i = 0; i < 10 && !alpsOutline; i += 1) {
      ...
      alpsOutline = await page.evaluate(() => {
        ...
        const items = window.__r1Annotations?.list?.() || [];
    ...
    const heading = () => page.evaluate(() => window.__r1.viewer.camera.heading * 180 / Math.PI);
    ...
    if (r?.ok) {
      ...
      const trackingReleased = await page.evaluate(() => !window.__r1.viewer.trackedEntity);
    ...
    if (trackId2 && (await run('track_entity', { query: trackId2, layerId: 'flights' }))?.ok) {
      ...
      await settle(6000);
      const stillTracked = await page.evaluate(() => Boolean(window.__r1.viewer.trackedEntity));
    ...
    await settle(800);
    annoCount = await page.evaluate(() => window.__r1Annotations?.list?.().length ?? window.__r1Annotations?.count?.() ?? -1);
    ...
    const contextEntered = await page.evaluate(
      () => window.__r1?.styleManager?.getContextModeState?.() || null,
    );
    ...
    const cockpitBefore = await page.evaluate(
      () => Boolean(window.__r1?.styleManager?.cockpitView?.active),
    );
    ...
    const awarenessFlights = await page.evaluate(() => {
      const snap = window.__r1?.dataManager?.layers
    ...
    // airliner.
    if (await page.evaluate(() => Boolean(window.__r1?.styleManager?.cockpitView?.active))) {
    ...
    const gated = await page.evaluate(
      () => window.__r1?.styleManager?.controlCockpit?.('enter') || null,
    );
