...
async function main() {
  ...
  await page.waitForFunction(
    () => window.__r1 && window.__r1.viewer && window.__r1.viewer.creditDisplay,
    { timeout: 60000 },
  );
  ...
  const registeredHtml = await page.evaluate(() => {
    const cd = window.__r1.viewer.creditDisplay;
  ...
  await page.evaluate(() => window.__r1.viewer.creditDisplay.hideLightbox());
  await page.screenshot({ path: resolve(SHOT_DIR, 'attribution-stacking-before-desktop.png') });
  await page.evaluate(() => {
    window.__r1.viewer.creditDisplay.showLightbox();
  ...
  await page.evaluate(() => {
    window.__r1.viewer.creditDisplay.hideLightbox();
    document.querySelector('[data-qa-attribution-stack-probe]')?.remove();
  });
  ...
  if (dcId) await page.evaluate((id) => window.__r1.dataManager.setEnabled(id, true), dcId);
  if (cableId) await page.evaluate((id) => window.__r1.dataManager.setEnabled(id, true), cableId);
  await new Promise((r) => setTimeout(r, 800));
  const afterEnableHtml = await page.evaluate(() =>
    (window.__r1.viewer.creditDisplay._staticCredits || []).map((c) => c.html),
  );
  check(
