...
function settleTraffic(page, view, { minCount = 100, timeoutS = 45 } = {}) {
  return page.evaluate(async (v, minC, tS) => {
    ...
    const mod = r1.dataManager.layers.get('traffic').module;
    try { r1.viewer.camera.cancelFlight(); } catch { /* no flight */ }
  ...
  try {
    ...
    for (const view of VIEWS) {
      ...
      for (const style of STYLES) {
        ...
        // the r1:style-change event the traffic layer listens to. The
        // thermal palette uniform selects WHOT vs Ironbow.
        await page.evaluate((s) => {
          const sm = window.__r1.styleManager;
        ...
        for (const detect of ['off', 'on']) {
          await page.evaluate((d) => {
            window.__r1.styleManager._setDetectionMode(d === 'on' ? 'DENSE' : 'OFF');
          }, detect);
          await sleep(1200); // label-arbiter solve + a few rendered frames

          const final = await page.evaluate(() => {
            const mod = window.__r1.dataManager.layers.get('traffic').module;
