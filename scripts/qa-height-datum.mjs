 ...
 *      (`r1:map-stack-changed` → cctv.js's regime re-arm — see its Task 5
...
async function main() {
  ...
  try {
    ...
    const allCameras = await page.evaluate(() => window.__r1.dataManager.layers.get('cctv').module.getUIState().cameras);
    ...
    async function checkActiveCameraBand(cityLabel, targetCamId, band) {
      await page.evaluate((id) => {
        window.__r1.dataManager.layers.get('cctv').module.selectCamera(id, { focus: true, durationSec: 0.1 });
      }, targetCamId);
      await sleep(600);
    ...
    const gotAircraft = await page.waitForFunction(
      () => {
        const mod = window.__r1.dataManager.layers.get('flights').module;
    ...
    if (!gotAircraft) {
      ...
      const nearbyList = await page.evaluate(() => {
        const mod = window.__r1.dataManager.layers.get('flights').module;
        const center = window.__r1.viewer.camera.position;
    ...
    await page.evaluate(async () => {
      await window.__r1.mapStackController.setStack('osm');
    });
    ...
    try {
      await page.waitForFunction(() => {
        const p = window.__r1?.viewer?.terrainProvider;
    ...
    terrainCtor = await page.evaluate(() => {
      const provider = window.__r1.viewer.terrainProvider;
