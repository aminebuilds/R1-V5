 ...
 *                  the __R1_ENRICH_AMBIENT_QA seam; a real 5-min window can't
...
// Rolling-budget QA seam (window.__R1_ENRICH_AMBIENT_QA in flights.js):
...
async function main() {
  ...
  try {
    ...
    await page.evaluateOnNewDocument((spec) => {
      ...
      window.__R1_ENRICH_AMBIENT_QA = { ...spec.budgetQa };
    ...
    await page.evaluate(async () => {
      const dm = window.__r1.dataManager;
      await dm.layers.get('flights').module.update(window.__r1.viewer);
    });
    await sleep(1000);
    ...
    await page.evaluate((winMs) => { window.__R1_ENRICH_AMBIENT_QA.windowMs = winMs; }, RESUME_WINDOW_MS);
