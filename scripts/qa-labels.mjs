...
const STORAGE_KEY = 'r1:detection-allocation:v1';
...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(
      () => window.__r1?.viewer && window.__r1?.styleManager,
    ...
    const injected = await page.evaluate(({ fieldCounts, normalCounts }) => {
      const { viewer, dataManager, styleManager } = window.__r1;
    ...
    await page.waitForFunction(
      () => window.__r1.styleManager.getDetectionDiagnostics()?.observationCount === 5200,
      { timeout: SAMPLE_TIMEOUT_MS, polling: 100 },
    );
    await page.evaluate(() => {
      const viewer = window.__r1.viewer;
