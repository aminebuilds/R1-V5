...
try {
  ...
  await page.waitForFunction(() => !!window.__r1?.viewer, { timeout: 90_000 });
  ...
  await page.evaluate(async () => {
    ...
    for (const [id, entry] of r1.dataManager.layers) {
      if (entry.enabled) { try { await r1.dataManager.setEnabled(id, false, { origin: 'user' }); } catch { /* gate reports via counts */ } }
    }
  });
  ...
  const diag = () => page.evaluate(() => window.__r1.getRenderGovernorDiagnostics?.()
    || window.__r1RenderGovernor?.getDiagnostics?.() || null);
  ...
  const detectionDefault = await page.evaluate(
    () => window.__r1.styleManager.getDetectionState?.() || null,
  );
  check(
  ...
  await page.evaluate(() => { window.__r1.styleManager._setDetectionMode('OFF'); });
  ...
  await page.evaluate(() => { window.__r1.styleManager._setDetectionMode('DENSE'); });
  ...
  const detectPainted = await page.evaluate(() => new Promise((resolve) => {
    const r1 = window.__r1;
    const before = r1.styleManager.getDetectionDiagnostics?.()?.frameCount ?? null;
    let paints = 0;
    ...
    let last = r1.styleManager.getDetectionDiagnostics?.();
    const remove = r1.viewer.scene.postRender.addEventListener(() => {
      const now = r1.styleManager.getDetectionDiagnostics?.();
      if (now && now !== last) { paints += 1; last = now; }
    });
    let steps = 0;
    const id = setInterval(() => {
      r1.viewer.camera.moveForward(50);
      steps += 1;
      if (steps >= 20) {
        clearInterval(id);
        setTimeout(() => {
          remove();
          resolve({ paints, before, after: r1.styleManager.getDetectionDiagnostics?.()?.frameCount ?? null });
        }, 400);
      }
    }, 60);
  }));
  ...
  await page.evaluate(() => { window.__r1.styleManager.setStyle('retro'); });
  ...
  await page.evaluate(() => { window.__r1.styleManager.setStyle('normal'); });
  ...
  const detectionStillOn = await page.evaluate(
    () => window.__r1.styleManager.getDetectionState?.()?.detectionMode || null,
  );
  check(
  ...
  await page.evaluate(async () => {
    await window.__r1.dataManager.setEnabled('satellites', true, { origin: 'user' });
  });
  const dSat = await diag();
  check('satellites enable registers its holder', dSat?.holds.includes('satellites'), dSat);
  await page.evaluate(async () => {
    await window.__r1.dataManager.setEnabled('satellites', false, { origin: 'user' });
  });
  ...
  await page.evaluate(async () => {
    await window.__r1.dataManager.setEnabled('flights', true, { origin: 'user' });
  });
  ...
  await page.evaluate(async () => {
    await window.__r1.dataManager.setEnabled('flights', false, { origin: 'user' });
  });
