...
try {
  ...
  await page.evaluate(async () => {
    ...
    for (const [id, entry] of r1.dataManager.layers) {
      if (entry.enabled) { try { await r1.dataManager.setEnabled(id, false, { origin: 'user' }); } catch { /* measured via counts */ } }
    }
  });
  ...
  if (!control) {
    const labelLatency = await page.evaluate(async (layerId, isLegacy) => {
      ...
      await r1.dataManager.setEnabled(layerId, true, { origin: 'user' });
      const deadline = t0 + 60_000;
      const labelsUp = () => {
        if (isLegacy) {
          const stats = r1.dataManager.layers.get(layerId)?.module?.getStats?.() || {};
  ...
  if (!legacy && !control) {
    ...
    const cycle = await page.evaluate(async (layerId) => {
      const r1 = window.__r1;
      await r1.dataManager.setEnabled(layerId, false, { origin: 'user' });
      r1.viewer.scene.requestRender?.();
      await new Promise((r) => setTimeout(r, 1_200));
      const offDiag = window.__r1WorldOverlay?.getDiagnostics?.() || {};
