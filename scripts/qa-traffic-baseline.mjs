...
async function moveCamera(page, view) {
  return page.evaluate((v) => {
    ...
    const module = r1.dataManager.layers.get('traffic').module;
    ...
    try { r1.viewer.camera.cancelFlight(); } catch { /* no active flight */ }
    ...
    r1.viewer.camera.moveEnd.raiseEvent();
...
async function waitForTrafficCapture(page, beforeLastUpdate, boundaryTime) {
  await page.waitForFunction(
    (before, boundary) => {
      const module = window.__r1?.dataManager?.layers?.get('traffic')?.module;
...
function readRenderer(page) {
  return page.evaluate(() => {
    const gl = window.__r1?.viewer?.scene?.context?._gl;
...
async function main() {
  ...
  try {
    ...
    await page.evaluate(async (highView) => {
      const r1 = window.__r1;
      await r1.dataManager.restoreEnabledLayerIds([]);
      const ellipsoid = r1.viewer.scene.globe.ellipsoid;
      const radians = Math.PI / 180;
      r1.viewer.camera.setView({
        destination: ellipsoid.cartographicToCartesian({
      ...
      await r1.dataManager.setEnabled('traffic', true);
      r1.viewer.camera.changed.raiseEvent();
      r1.viewer.camera.moveEnd.raiseEvent();
    }, VIEWS.austin);
    await sleep(500);
    ...
    const timingDiagnostics = await page.evaluate(() => (
      window.__r1.dataManager.layers.get('traffic').module.getStats().trafficTiming
    ));
