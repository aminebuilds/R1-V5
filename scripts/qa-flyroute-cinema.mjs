...
async function installSampler() {
  await page.evaluate(() => {
    ...
    window.__r1FlyTrace = { rows: [], marks: [] };
    if (window.__r1FlyTraceRemove) window.__r1FlyTraceRemove();
    let frame = 0;
    const listener = () => {
    ...
    window.__r1FlyTraceRemove = () => viewer.scene.postRender.removeEventListener(listener);
  });
}

async function readTrace() {
  return page.evaluate(() => ({
    rows: window.__r1FlyTrace.rows.slice(),
    marks: window.__r1FlyTrace.marks.slice(),
  }));
}
...
try {
  ...
  const run = (name, args = {}) => page.evaluate(
    (n, a) => window.__r1VoiceCommands.runner(n, a), name, args,
  );
  ...
  const routeInfo = await page.evaluate(() => {
    const route = (window.__r1Annotations.list?.() || []).filter((a) => a.type === 'route').at(-1);
  ...
  await page.evaluate(() => window.__r1FlyTrace.marks.push({ label: 'flight-start', t: performance.now() }));
  ...
  const stillForMs = () => page.evaluate(() => {
    const rows = window.__r1FlyTrace.rows;
  ...
  await page.evaluate(() => window.__r1FlyTraceRemove?.());
  ...
  const liveRollDeg = () => page.evaluate(
    () => (window.__r1.viewer.camera.roll * 180) / Math.PI,
  );
  let rollBeforeCut = 0;
  ...
  const cut = await page.evaluate(async () => {
    ...
    window.__r1FlyTrace.marks.push({ label: 'pointerdown', t: performance.now() });
