...
async function positionPointerForGizmoDrag(page, point, frameCount = 3) {
  await page.evaluate(() => {
    window.__r1.dataManager.layers.get('cctv').module
      .setParams({ calibrationMode: false });
  });
  await page.mouse.move(point.x, point.y);
  return page.evaluate(async (count) => {
    const r1 = window.__r1;
    r1.dataManager.layers.get('cctv').module.setParams({ calibrationMode: true });
...
function waitForTilesLoaded(page, timeoutMs = 15000) {
  return page.waitForFunction(
    () => {
      const scene = window.__r1.viewer.scene;
...
async function main() {
  ...
  try {
    ...
    const camCount = await page.evaluate(() => window.__r1.dataManager.layers.get('cctv').module.getUIState().count);
    ...
    const drained = await page.waitForFunction(
      () => {
        const mod = window.__r1.dataManager.layers.get('cctv').module;
    ...
    const activeIdBeforeActivation = await page.evaluate(() => window.__r1.dataManager.layers.get('cctv').module.getUIState().activeCameraId);
    ...
    const activeId = await page.evaluate(() => window.__r1.dataManager.layers.get('cctv').module.getUIState().activeCameraId);
    ...
    await page.evaluate((id) => {
      window.__r1.dataManager.layers.get('cctv').module.setParams({ selectedCameraId: id });
    }, activeId);
    await sleep(400);
    ...
    const geomInfo = await page.evaluate(`(function(camId){
      ...
      const diag = window.__r1WorldOverlay?.getDiagnostics?.();
    ...
    const baseStoreEmpty = await page.evaluate((camId) => {
      const raw = localStorage.getItem('r1.cctv.calibration.v2');
    ...
    const clickEvidenceSetup = await page.evaluate(() => {
      const r1 = window.__r1;
      const viewer = r1.viewer;
      const mod = r1.dataManager.layers.get('cctv').module;
      const snapshotPose = () => ({
      ...
      window.addEventListener('r1:cctv-request-focus', evidence.onFocus);
      evidence.dispose = () => {
        evidence.unsubscribe?.();
        window.removeEventListener('r1:cctv-request-focus', evidence.onFocus);
      };
    ...
    const siblingClick = await page.evaluate(() => {
      ...
      if (window.__qaCctvSiblingOwner) {
        r1.viewer.entities.remove(window.__qaCctvSiblingOwner);
    ...
    const firstEmptyClick = await page.evaluate(() => {
      const evidence = window.__qaCctvClickEvidence;
      const state = window.__r1.dataManager.layers.get('cctv').module.getUIState();
      return {
    ...
    const countVolumes = () => page.evaluate(() => {
      const prims = window.__r1.viewer.scene.primitives;
      let n = 0;
      for (let i = 0; i < prims.length; i++) {
        if (prims.get(i) && prims.get(i)._r1Viewshed) n += 1;
      }
      return n;
    });
    ...
    await page.evaluate(() => {
      ...
      for (let i = 0; i < prims.length; i++) {
        if (prims.get(i) && prims.get(i)._r1Viewshed) window.__qaViewshedRefs.push(prims.get(i));
      }
    });
    await sleep(8000);
    const viewshedIdle = await page.evaluate(() => {
      ...
      const now = [];
      for (let i = 0; i < prims.length; i++) {
        if (prims.get(i) && prims.get(i)._r1Viewshed) now.push(prims.get(i));
      }
