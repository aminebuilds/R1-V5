...
async function waitForCardCanvasInk(page, { timeoutMs = 12000 } = {}) {
  ...
  while (Date.now() < deadline) {
    await page.evaluate(() => window.__r1?.viewer?.scene?.requestRender?.());
    await sleep(150);
...
async function bootAndEnable(page, { timeoutS = 45 } = {}) {
  ...
  await page.waitForFunction(
    () => window.__r1?.viewer && window.__r1?.dataManager,
    { timeout: 60000 },
  );
  await sleep(1500);
  ...
  try {
    ...
    console.log('\n(ii) CARDS — tactical card overlay ink at both LODs...');
    {
      const strongest = await page.evaluate(() => {
        const mod = window.__r1.dataManager.layers.get('local-firms').module;
    ...
    if (!(actionCards.present && actionCards.painted > 0 && beforeActions.count > 0)) {
      ...
      const prepared = await page.evaluate(() => {
        const r1 = window.__r1;
        const mod = r1.dataManager.layers.get('local-firms').module;
        const target = mod.getDetectableObjects({ maxCount: 1 })[0];
        if (!target?.position) return null;

        const sentinel = r1.viewer.entities.add({
          id: 'qa-firms-prior-camera-owner',
          position: target.position,
        });
        r1.viewer.trackedEntity = sentinel;
        ...
        const proof = {
          ...
          request: null,
          generationBefore: r1.styleManager._navigationGeneration,
        };
        ...
        window.addEventListener('r1:world-request-focus', proof.onRequest);
        camera.flyToBoundingSphere = function qaFirmsObservedFlight(...args) {
          proof.flightCount += 1;
          proof.trackedAtFlight = r1.viewer.trackedEntity?.id || null;
      ...
      if (!prepared) {
        ...
        const proof = await page.evaluate(() => {
          ...
          const selectedEntityId = window.__r1ContextStore?.selectedEntityId || null;
          const result = {
            flightCount: state?.flightCount || 0,
            trackedAtFlight: state?.trackedAtFlight,
            trackingReleased: r1.viewer.trackedEntity == null,
            generationBefore: state?.generationBefore,
            generationAfter: r1.styleManager._navigationGeneration,
          ...
          if (state) {
            r1.viewer.camera.flyToBoundingSphere = state.originalFly;
            window.removeEventListener('r1:world-request-focus', state.onRequest);
            if (state.sentinel) r1.viewer.entities.remove(state.sentinel);
          }
