...
const appState = (page) => page.evaluate(() => {
  ...
  return {
    ...
    durable: localStorage.getItem('r1:first-run-mission:v1'),
    session: sessionStorage.getItem('r1:first-run-mission-session:v1'),
    layerStateBlob: localStorage.getItem('r1:layer-state:v2'),
    allocation: localStorage.getItem('r1:detection-allocation:v1'),
  };
});
...
async function main() {
  ...
  try {
    ...
    await section('show-policy', async () => {
      ...
      const shareState = await page.evaluate(() => !!window.__r1?.styleManager?.hasShareState);
