...
async function readTileReadiness(page) {
  return page.evaluate(() => {
    ...
    const controller = r1.mapStackController;
    const activeStack = controller?.getActiveId?.() || null;
    const tileset = controller?.googleTileset || r1.tileset || null;
  ...
  const result = await page.evaluate(async (timeoutMs) => {
    ...
    try {
      while (Date.now() < deadline) {
        r1.viewer.scene.render(r1.viewer.clock.currentTime);
...
async function capture(page, scenario, frame) {
  ...
  const dataUrl = await page.evaluate(() => (
    window.__r1.viewer.scene.canvas.toDataURL('image/png')
  ));
...
async function takeFrameClock(page) {
  const result = await page.evaluate(() => {
    const seam = window.__r1.dataManager.layers.get('flights').module.__focusEvidence;
...
async function advanceEvidenceFrame(page, deltaMs = FRAME_MS) {
  return page.evaluate((stepMs) => {
    ...
    const seam = r1.dataManager.layers.get('flights').module.__focusEvidence;
...
async function waitForApp(page) {
  ...
  await page.waitForFunction(
    () => Boolean(window.__r1?.viewer && window.__r1?.dataManager),
...
async function setBasemap(page, stackId) {
  const result = await page.evaluate(async (id) => (
    window.__r1.styleManager.setMapStack(id)
  ), stackId);
  if (!result?.ok) {
...
async function enableLayer(page, layerId) {
  await page.evaluate(async (id) => {
    await window.__r1.dataManager.setEnabled(id, true);
  }, layerId);
}
...
async function injectAndTrackTarget(page, extraAircraft = [], target = TARGET) {
  ...
  await page.waitForFunction(
    () => Boolean(window.__r1.viewer.trackedEntity),
    { timeout: 10_000 },
  );
...
async function readTrackedBasis(page) {
  return page.evaluate((targetId) => {
    const r1 = window.__r1;
    const layer = r1.dataManager.layers.get('flights').module;
    ...
    const canvasHeight = r1.viewer.scene.canvas.clientHeight;
...
async function flightSnapshot(page) {
  return page.evaluate(() => (
    window.__r1.dataManager.layers.get('flights').module.__focusEvidence.snapshot()
  ));
}

async function setTuning(page, params) {
  return page.evaluate((next) => (
    window.__r1.dataManager.layers.get('flights').module.__focusEvidence.setTuning(next)
  ), params);
}
...
async function vesselRowsAroundTarget(page) {
  return page.evaluate(() => {
    ...
    const Cartographic = r1.viewer.camera.positionCartographic.constructor;
    const ellipsoid = r1.viewer.scene.globe.ellipsoid;
    const rows = [];
...
async function runHarborClutter(page, effectiveParams) {
  ...
  await page.evaluate((vessels) => {
    const seam = window.__r1.dataManager.layers
...
async function runHorizonBand(page, effectiveParams) {
  ...
  await page.evaluate((records) => {
    window.__r1.dataManager.layers.get('flights')
