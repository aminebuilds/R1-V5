# R.1 — Testing Guide (voice map-annotation whiteboard + live tracking)
...
- **Voice (the real feature):** click **R1 MIC** (bottom of screen) → wait for **LISTENING** →
...
- **Console (deterministic, no mic/live-data needed):** open DevTools (**Cmd-Opt-J**) and use:
  - `window.__r1Annotations.tour()` — self-running narrated SF tour (camera + marks in sequence)
  - `window.__r1Annotations.demo()` — lays the SF set down at once
  - `window.__r1Annotations.clear()` — erase all marks
  - `window.__r1Annotations.count()` — how many marks are live
...
1. While tracking a plane over Austin, R1 MIC: *"annotate downtown Austin."*
...
> All from the Austin default view (no camera move needed). Click R1 MIC, then speak.
...
- Sanity in console: `window.__r1Annotations.count()` → `0` after a clear.
...
1. Console: `window.__r1Annotations.tour()`. 📸 A couple of frames as it runs.
...
1. Console:
   `window.__r1Annotations.annotate([{type:'area',target:'Travis County, Texas',label:'Travis County',color:'green',footprint:true}],{flyTo:true,persist:true})`
...
- **No R1 MIC button / voice errors:** `OPENAI_API_KEY` didn't load — use the console API for
