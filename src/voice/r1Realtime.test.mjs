...
import { controlRadio, createR1ActionRunner } from './r1Actions.js';
import {
  computeDownscale,
  renderFreshCesiumFrame,
  estimateDataUrlBytes,
  R1RealtimeController,
...
test('replacement input invalidates an in-flight Radio preflight immediately', () => {
  const order = [];
  const controller = new R1RealtimeController({
    ui: {},
    runner: async () => ({}),
...
test('Radio handoff waits for response.done so later multi-intent tools execute before voice closes', async () => {
  ...
  const controller = new R1RealtimeController({ runner, ui, radioLayer });
  controller.debugLog = () => {};
...
test('generic same-response Radio visibility disable supersedes delayed Select', async () => {
  ...
  const genericRunner = createR1ActionRunner({ viewer, styleManager: {}, dataManager });
  const ui = {
    root: { dataset: {}, classList: { remove() {} }, querySelectorAll: () => [] },
    status: { textContent: '' },
    detail: { textContent: '', title: '' },
        ...
        for (const controlSucceeds of [true, false]) {
          await t.test(
            ...
            async () => {
              ...
              const genericRunner = createR1ActionRunner({
                viewer,
                styleManager: {},
                dataManager,
              });
...
test('an unset or hand-edited tier reads back as mini', () => {
  ...
  assert.equal(
    readStoredVoiceTier(fakeVoiceStorage({ 'r1.voiceCost.tier': 'gpt-4o' })),
    'mini'
  );
  assert.equal(
    readStoredVoiceTier(fakeVoiceStorage({ 'r1.voiceCost.tier': '__proto__' })),
    'mini'
  );
});
...
test('writing a bogus tier persists the safe fallback, not the bogus value', () => {
  ...
  assert.equal(storage.dump()['r1.voiceCost.tier'], 'mini');
});
...
test('corrupt stored limits fall back to defaults rather than disarming the cap', () => {
  ...
  const limits = readStoredVoiceLimits(
    fakeVoiceStorage({ 'r1.voiceCost.limits': '{oops' })
  );
...
test('partially stored limits keep the default for the missing threshold', () => {
  const limits = readStoredVoiceLimits(
    fakeVoiceStorage({ 'r1.voiceCost.limits': '{"warnUsd":0.5}' })
  );
...
test('a disabled threshold survives the storage round-trip', () => {
  ...
  const raw = storage.dump()['r1.voiceCost.limits'];
...
test('a typed command drops the old response’s queued follow-up confirmation', () => {
  ...
  controller.recordUsage = () => {};
  controller.updateResponseState({ type: 'response.created', response: { id: 'resp_old' } });
  controller.queueResponseCreate('Briefly confirm the completed R1 action once.');
