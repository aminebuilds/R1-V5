 ...
 *    a chip label into a permanent two-child shell: a `.r1-flap-text` span
 *    holding one long-lived `Text` node, and an `aria-hidden`
 *    `.r1-flap-cells` sibling. After that the ONLY text operation for the
 ...
 *    staggered via a per-cell `--r1-flap-delay` custom property. A change
 ...
 * `r1-flap-out`/`r1-flap-in` keyframe crossover in style.css — it is what
 * makes an interrupted cascade pick honest outgoing glyphs.
 */
export const FLAP_TURN_RATIO = 0.5;

const HOST_CLASS = 'r1-flap-host';
const ACTIVE_CLASS = 'r1-flap-active';
const TEXT_CLASS = 'r1-flap-text';
const CELLS_CLASS = 'r1-flap-cells';
const CELL_CLASS = 'r1-flap-cell';
const FLAPPING_CLASS = 'is-flapping';
const SIZING_CLASS = 'r1-flap-sizing';
 ...
 * `.r1-flap-text` is the same object for the life of the chip, so a label
...
function clearSizing(element) {
  ...
  element.style?.removeProperty('--r1-flap-total');
}
...
function easeWidth(element, fromWidth, toWidth, durationMs) {
  ...
  element.style.setProperty('--r1-flap-total', `${durationMs}ms`);
...
function rest(element, host) {
  ...
  element.style?.removeProperty('--r1-flap-dur');
...
export function setSplitFlapText(element, text, options = {}) {
  ...
  for (const cell of plan.cells) {
    ...
    if (cell.changed) {
      ...
      node.style.setProperty('--r1-flap-delay', `${cell.delayMs}ms`);
    }
    cellNodes.push(node);
  }
  ...
  element.style.setProperty('--r1-flap-dur', `${charMs}ms`);
