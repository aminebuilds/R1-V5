 ...
 * @module cctv
 *
 * CCTV camera data layer for R.1.
 *
 * Architecture:
...
export const CCTV_CALIBRATION_STORAGE_KEY_V1 = 'r1.cctv.calibration.v1';
/** v2 store key. Entries: { values: <7-field calibration offsets>, source: 'manual', savedAt: <epoch ms> }. */
export const CCTV_CALIBRATION_STORAGE_KEY_V2 = 'r1.cctv.calibration.v2';
...
// Task 5: window listener handle for the 'r1:map-stack-changed' CustomEvent
...
/**
 * Task 5: surface-regime change handler ('r1:map-stack-changed'
...
function rebuildViewshedVolume(record, isActive) {
  ...
  primitive._r1Viewshed = record.camera.id;
...
const cctvLayer = {
  ...
  async init(viewer) {
    ...
    if (!_mapStackListener && typeof window !== 'undefined') {
      ...
      window.addEventListener('r1:map-stack-changed', _mapStackListener);
    }
  ...
  destroy(viewer) {
    unregisterPickOwner('cctv');
    if (_mapStackListener && typeof window !== 'undefined') {
      window.removeEventListener('r1:map-stack-changed', _mapStackListener);
