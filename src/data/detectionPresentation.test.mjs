...
test('detection owns and releases the Cockpit lifecycle listener', async () => {
  ...
  assert.match(source, /addEventListener\('r1:cockpit-mode-changed', _cockpitModeListener\)/);
  assert.match(source, /removeEventListener\('r1:cockpit-mode-changed', _cockpitModeListener\)/);
  assert.match(
    source,
