...
test('military poll refreshes tracked callsign/altitude/kts and marks a missed poll STALE', async () => {
  ...
  try {
    ...
    assert.equal(entity.r1LabelModel.title, 'RCH451');
    assert.match(entity.r1LabelModel.details.join(' · '), /28000 ft/);
    assert.match(entity.r1LabelModel.details.join(' · '), /400 kt/);

    await militaryFlightsLayer.update(viewer);
    assert.match(entity.r1LabelModel.title, /STALE/);
  } finally {
    globalThis.fetch = realFetch;
  }
});
