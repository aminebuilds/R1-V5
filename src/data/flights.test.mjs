...
test('nonempty OpenSky payload with zero usable rows cannot prove share target absence', async () => {
  _setTrackedFlightRefreshStateForTest({
    icao24: 'abc123',
    entity: { r1LabelModel: { title: 'WARM', details: [] } },
...
test('flights poll refreshes tracked callsign/FL/kts and marks a missed poll STALE', async () => {
  const icao24 = 'a1b2c3';
  const entity = { r1LabelModel: { title: 'OLD', details: [] } };
  const billboard = {
  ...
  try {
    ...
    assert.equal(entity.r1LabelModel.title, 'DAL123');
    assert.match(entity.r1LabelModel.details.join(' · '), /FL350/);
    assert.match(entity.r1LabelModel.details.join(' · '), /486 kts/);

    await flightsLayer.update(viewer);
    assert.match(
      [entity.r1LabelModel.title, ...entity.r1LabelModel.details].join(' · '),
      /STALE/,
    );
  } finally {
    globalThis.fetch = realFetch;
...
test('real civil track path creates no native label and publishes every cached host line', async () => {
  ...
  globalThis.window.addEventListener('r1:awareness-subject-selected', (event) => {
  ...
  try {
    ...
    assert.deepEqual(entity.r1LabelModel, {
    ...
    assert.equal(entity.r1SelectionOrigin, 'user');
    ...
    assert.equal(entry.title, entity.r1LabelModel.title);
    assert.deepEqual(entry.details, entity.r1LabelModel.details);
...
// → `r1:awareness-subject-selected`), consumed by Context/awareness, and it
...
test('civil tracked readout: a callsign-less enriched contact reads as its registration', async () => {
  ...
  try {
    await flightsLayer.update({ camera: { positionCartographic: null }, scene: {} });
    assert.match(entity.r1LabelModel.title, /^N123AB\b/);
    assert.doesNotMatch(
      [entity.r1LabelModel.title, ...entity.r1LabelModel.details].join(' · '),
      /ae1fa4/,
    );
