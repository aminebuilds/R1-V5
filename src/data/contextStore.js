const STORE_KEY = '__r1ContextStore';
...
export function registerEntityContext(entity, metadata) {
  ...
  entity.__r1ContextId = metadata.id;
...
export function selectEntityContext(entity) {
  const store = getContextStore();
  const contextId = entity?.__r1ContextId;
  ...
  window.dispatchEvent(new CustomEvent('r1:entity-selected', { detail: record }));
  return record;
}
 ...
 *
 * Deliberately does NOT dispatch `r1:entity-selected`: tracking layers own a
 * separate publication lane (`r1:awareness-subject-selected`) that the
...
export function selectTrackedSubjectContext(metadata) {
  ...
  const carrier = store.entities.get(id)?.entity || { __r1ContextId: id };
 ...
 * same reason: `r1:awareness-subject-cleared` is the tracking layers' lane.
...
export function clearSelectedEntityContextForLayer(layerId, { evicted = false } = {}) {
  ...
  if (record?.layerId === layerId) {
    store.selectedEntityId = null;
    store.selectedAt = null;
    window.dispatchEvent(new CustomEvent('r1:entity-selection-cleared', {
