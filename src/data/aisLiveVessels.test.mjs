...
test('vessel interaction wire: trail pick does not deselect', () => {
  const harness = installWireHarness({ id: 'r1-trail:71' });
  try {
...
test('vessel interaction wire: only the r1-trail: namespace receives the trail no-op', () => {
  const harness = installWireHarness({ id: 'r1-trailing-contact' });
  try {
    harness.handler.click({ position: { x: 10, y: 20 } });
...
test('vessel interaction wire: id-less 3D Tiles pick deselects and resets the HUD', () => {
  ...
  try {
    ...
    harness.windowTarget.addEventListener('r1:entity-selection-cleared', (event) => {
...
test('vessel interaction wire: empty pick deselects and emits r1:entity-selection-cleared', () => {
...
test('vessel real layer lifecycle publishes protected selection and leaves no stale host cards', () => {
  ...
  globalThis.document = { getElementById: () => null };
  const focusRequests = [];
  globalThis.window.addEventListener('r1:world-request-focus', (event) => {
...
test('vessel interaction wire: deselecting never moves the camera', () => {
  const harness = installWireHarness({ id: 'r1-empty-space' });
  const requests = [];
  harness.windowTarget.addEventListener(
    WORLD_FOCUS_REQUEST_EVENT,
    (event) => requests.push(event.detail),
