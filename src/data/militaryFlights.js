 ...
 * no `_modelDisplayPosition` from postRender, and `r1DisplayPosition` keeps its
...
function _startTrail(icao24) {
  ...
  if (!_trailHeadEntity && _viewer) {
    _trailHeadEntity = _viewer.entities.add({
      ...
      id: `r1-trail:mil-head-${++_trailHeadSeq}`,
...
function _trackFlight(icao24, { origin = 'programmatic' } = {}) {
  ...
  _trackedEntity.r1TrackedId = `military:${icao24}`;
  _trackedEntity.r1LabelModel = trackedLabelModelFromText(
  ...
  // Separate accessor on purpose (mirror of flights.js): `r1DisplayPosition` keeps the
  // follow-camera anti-jitter contract; presentation that must weld to the aircraft you
  // can see reads `r1VisualPosition`.
