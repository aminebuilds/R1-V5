...
export function cachedTrackedDisplayPosition(entity) {
  if (!entity || typeof entity.r1DisplayPosition !== 'function') return null;
  try {
    return entity.r1DisplayPosition() || null;
  } catch {
    return null;
  }
}
 ...
 * `r1VisualPosition`, which returns the translation that model is actually rendering
 ...
 * `r1DisplayPosition`: that accessor carries the follow-camera anti-jitter contract
 * and must keep returning the value the camera settled on.
 * @param {Object|null} entity Tracked or selected presentation entity.
 * @returns {Object|null}
 */
export function cachedTrackedVisualPosition(entity) {
  if (entity && typeof entity.r1VisualPosition === 'function') {
    try {
      const visual = entity.r1VisualPosition();
...
function entryIdFor(entity) {
  const explicit = String(entity?.r1TrackedId || '').trim();
...
export function createTrackedOverlayEntry(entity) {
  const model = entity?.r1LabelModel;
  const id = entryIdFor(entity);
  if (!id || !model || typeof entity.r1DisplayPosition !== 'function') {
    return null;
  }
...
/**
 * Republish a layer's newly assigned `r1LabelModel` when that entity owns the
...
export function initTrackedReadout(viewer) {
  ...
  window.addEventListener('r1:awareness-subject-selected', _aircraftSelectedHandler);
  syncActiveEntity();
...
export function destroyTrackedReadout() {
  ...
  if (_aircraftSelectedHandler) window.removeEventListener('r1:awareness-subject-selected', _aircraftSelectedHandler);
