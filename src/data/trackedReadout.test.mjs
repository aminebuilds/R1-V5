...
test('tracked position source reads only r1DisplayPosition and never entity.position', () => {
  const cached = { x: 1, y: 2, z: 3 };
  let propertyReads = 0;
  const entity = {
    r1DisplayPosition: () => cached,
    position: {
...
test('the tracked card anchors to the visual position without repurposing the display accessor', () => {
  ...
  // model you can see, while `r1DisplayPosition` keeps returning the cached
  // dead-reckoned value the follow camera settled on (anti-jitter contract).
  const display = { x: 1, y: 2, z: 3 };
  const visual = { x: 1, y: 2, z: 103 };
  let displayReads = 0;
  const entity = {
    r1TrackedId: 'flights:aaa077',
    r1DisplayPosition: () => { displayReads += 1; return display; },
    r1VisualPosition: () => visual,
    r1LabelModel: { title: 'SWA143', details: ['ON GROUND'], accent: '#6be8ff' },
  };
  ...
  const spriteOnly = { r1DisplayPosition: () => display };
  assert.equal(cachedTrackedVisualPosition(spriteOnly), display);
  assert.equal(cachedTrackedVisualPosition({ ...spriteOnly, r1VisualPosition: () => null }), display,
    'a null visual position falls back rather than blanking the card');
  assert.equal(
    cachedTrackedVisualPosition({
      r1DisplayPosition: () => display,
      r1VisualPosition: () => { throw new Error('model gone'); },
    }),
    display,
...
test('tracked entry factory pins the production protected-lane policy', () => {
  ...
  const entry = createTrackedOverlayEntry({
    r1TrackedId: 'satellites:25544',
    r1DisplayPosition: () => display,
    r1LabelModel: { title: 'ISS', details: ['420 km · NORAD 25544'], accent: '#ffd84d' },
  });
...
test('tracked entity publishes a protected host entry backed by the frame cache', () => {
  ...
  const entity = {
    id: 'generated',
    r1TrackedId: 'flights:abc123',
    r1DisplayPosition: () => display,
    r1LabelModel: { title: 'UAL123', details: ['FL350 · 451 kts'], accent: '#39d0ff' },
  };
  ...
  try {
    ...
    entity.r1LabelModel = { ...entity.r1LabelModel, details: ['FL360 · 455 kts'] };
...
test('selection lifecycle ignores vessels, accepts installations, and clears without stale cards', () => {
  ...
  const recorder = makeHostRecorder();
  const installation = {
    r1TrackedId: 'installations:fort-test',
    r1DisplayPosition: () => ({ x: 1, y: 2, z: 3 }),
    r1LabelModel: { title: 'FORT TEST', details: ['AIRFIELD'], accent: '#5aa9ff' },
  };
  globalThis.window = fakeWindow;
  _setTrackedOverlayHostForTest(recorder.host);
  try {
    ...
    fakeWindow.dispatchEvent(new CustomEvent('r1:entity-selected', {
    ...
    fakeWindow.dispatchEvent(new CustomEvent('r1:entity-selection-cleared', {
...
test('tracking layers write r1LabelModel and expose only their cached display positions', async () => {
  ...
  for (const [name, source] of files) {
    assert.ok(source.includes('.r1LabelModel ='), `${name} writes the explicit model directly`);
    assert.ok(source.includes('.r1DisplayPosition ='), `${name} exposes a display-position cache`);
  }
  assert.ok(sources['flights.js'].includes('r1DisplayPosition = _trackedDisplayCached'));
  assert.ok(sources['militaryFlights.js'].includes('r1DisplayPosition = _trackedDisplayCached'));
  assert.ok(sources['satellites.js'].includes('r1DisplayPosition = _trackedDisplayCached'));
