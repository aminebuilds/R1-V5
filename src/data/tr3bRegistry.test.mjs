...
import { createR1ActionRunner } from '../voice/r1Actions.js';
...
test('a conversion survives a poll refresh, in both the billboard and the tracked card', async () => {
  ...
  try {
    ...
    assert.match(entity.r1LabelModel.title, /^DAL123 · FL350 · 486 kts$/);
    assert.deepEqual(entity.r1LabelModel.details.slice(0, 1), ['TR-3B'],
      'the tracked card class line reports TR-3B, replacing operator/type');
    assert.equal(
      [entity.r1LabelModel.title, ...entity.r1LabelModel.details].join(' · ').includes('Southwest'),
      false,
