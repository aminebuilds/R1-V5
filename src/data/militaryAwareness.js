...
function currentTrackedFlightSubject() {
  const trackedKey = normalizeContextId(state.viewer?.trackedEntity?.r1TrackedId);
...
function resolveSubjectPosition(subject, { allowCollectionMaterialization = true } = {}) {
  ...
  if (subject.layerId === 'flights' || subject.layerId === 'military') {
    const trackedPosition = state.viewer?.trackedEntity?.r1DisplayPosition?.();
    if (trackedPosition) {
      return {
        ...
        presence: String(state.viewer?.trackedEntity?.r1TrackedId || '') === subjectKey(subject)
...
function attachRuntimeListeners() {
  ...
  window.addEventListener('r1:awareness-subject-selected', state.subjectListener);
  window.addEventListener('r1:entity-selected', state.contextListener);
  window.addEventListener('r1:entity-selection-cleared', state.clearListener);
  window.addEventListener('r1:awareness-subject-cleared', state.subjectClearListener);
...
function detachRuntimeListeners() {
  if (state.runtimeListenersAttached) {
    window.removeEventListener('r1:awareness-subject-selected', state.subjectListener);
    window.removeEventListener('r1:entity-selected', state.contextListener);
    window.removeEventListener('r1:entity-selection-cleared', state.clearListener);
    window.removeEventListener('r1:awareness-subject-cleared', state.subjectClearListener);
  }
