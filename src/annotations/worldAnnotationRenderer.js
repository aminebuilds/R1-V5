...
export function createWorldAnnotationRenderer(viewer) {
  const dataSource = new Cesium.CustomDataSource('r1-annotations');
  viewer.dataSources.add(dataSource);

  // Register the R1RouteFlow fabric once so Cesium's Material.fromType() can build the
...
/** Register the R1RouteFlow fabric ONCE so Cesium's `Material.fromType('R1RouteFlow')`
...
function ensureFlowFabricRegistered() {
  ...
  makeRouteFlowMaterial('#ffffff'); // side effect: registers the 'R1RouteFlow' type
  _flowFabricRegistered = true;
}

/**
 * MaterialProperty for the animated route. Cesium builds the rendered Material once
 * from getType() (our registered R1RouteFlow fabric), then EACH FRAME calls
