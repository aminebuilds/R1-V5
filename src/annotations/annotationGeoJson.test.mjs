...
test('point (pin/highlight/label) round-trips, height preserved', () => {
  ...
  assert.equal(f.properties['r1:type'], 'pin');
...
test('area polygon round-trips: ring closed in GeoJSON, un-closed on import; centroid preserved', () => {
  ...
  assert.deepEqual(f.properties['r1:anchor'], [-122.434, 37.804]);
  assert.deepEqual(roundTrip(area), area); // ring back to 4, anchor from r1:anchor
});
...
test('synthesized + building props are preserved', () => {
  ...
  assert.equal(f.properties['r1:synthesized'], true);
...
test('malformed input fails CLOSED (returns null / skips)', () => {
  ...
  assert.equal(featureToAnnotation({ type: 'Feature', geometry: { type: 'Point', coordinates: [0, 0] }, properties: {} }), null); // no r1:type
  assert.equal(featureToAnnotation({ type: 'Feature', geometry: { type: 'Point', coordinates: [0, 0] }, properties: { 'r1:type': 'route' } }), null); // route needs LineString
  assert.equal(featureToAnnotation({ type: 'Feature', geometry: { type: 'LineString', coordinates: [[0, 0], [1, 1], [2, 2]] }, properties: { 'r1:type': 'arrow' } }), null); // arrow needs exactly 2
  assert.equal(featureToAnnotation({ type: 'Feature', geometry: { type: 'Polygon', coordinates: [[[0, 0], [1, 1]]] }, properties: { 'r1:type': 'area' } }), null); // <4 positions
  assert.equal(featureToAnnotation({ type: 'Feature', geometry: { type: 'Point', coordinates: ['x', 0] }, properties: { 'r1:type': 'pin' } }), null); // non-finite
