...
test('outline upgrade preserves the existing SVG group identity', (t) => {
  ...
  assert.ok(before.group.querySelector('.r1-anno-ring'), 'pending area starts as a reticle');
  ...
  assert.equal(after.group.querySelector('.r1-anno-ring'), null, 'reticle rings are removed in place');
  assert.ok(after.group.querySelector('.r1-anno-callout'), 'the original callout remains in the group');
  renderer.destroy();
});
...
test('an add that throws after inserting its group unwinds instead of orphaning it', (t) => {
  ...
  assert.equal(
    svg.children.filter((child) => child.classList.contains('r1-anno')).length,
    0,
