 ...
 *
 * Mapping (one Feature per mark; project fields namespaced `r1:`):
 *   pin | highlight | label  ->  Point      (anchor)
 *   arrow                     ->  LineString (anchor -> to)
 *   route                     ->  LineString (the routed path)
 *   area (with a ring)        ->  Polygon    (the footprint ring; centroid in `r1:anchor`)
...
export function annotationToFeature(anno) {
  ...
  const properties = {
    'r1:type': type,
    'r1:id': anno.id ?? null,
    'r1:label': anno.label ?? null,
    'r1:color': anno.color ?? null,
    'r1:ttlMs': anno.ttlMs ?? null,
  };
  let geometry = null;
  ...
  if (type === 'route') {
    ...
    properties['r1:mode'] = anno.mode ?? null;
    properties['r1:distanceM'] = anno.distanceM ?? null;
    properties['r1:durationS'] = anno.durationS ?? null;
    properties['r1:fallback'] = Boolean(anno.fallback);
  } else if (type === 'arrow') {
    ...
    properties['r1:footprintKind'] = anno.footprintKind ?? null;
    properties['r1:buildingHeight'] = anno.buildingHeight ?? null;
    properties['r1:synthesized'] = Boolean(anno.synthesized);
    const anchor = toPosition(anno.anchor);
    if (anchor) properties['r1:anchor'] = anchor; // preserve the exact centroid, don't recompute
  } else {
 ...
 * unknown `r1:type`, or geometry that doesn't match the declared type.
...
export function featureToAnnotation(feature) {
  ...
  const type = p['r1:type'];
  if (!VALID_TYPES.has(type)) return null;

  const base = {
    type,
    id: p['r1:id'] ?? null,
    label: p['r1:label'] ?? null,
    color: p['r1:color'] ?? 'primary',
    ttlMs: p['r1:ttlMs'] ?? null,
  };
  ...
  if (type === 'route') {
    ...
    return {
      ...
      path,
      mode: p['r1:mode'] ?? null,
      distanceM: p['r1:distanceM'] ?? null,
      durationS: p['r1:durationS'] ?? null,
      fallback: Boolean(p['r1:fallback']),
    };
  }
  ...
  if (g.type === 'Polygon') {
    ...
    const anchor = fromPosition(p['r1:anchor']) || ringCentroid(ring);
    return {
      ...base,
      anchor,
      to: null,
      ring,
      footprintKind: p['r1:footprintKind'] ?? null,
      buildingHeight: p['r1:buildingHeight'] ?? null,
      synthesized: Boolean(p['r1:synthesized']),
    };
  }
  ...
  if (type === 'area') {
    out.footprintKind = p['r1:footprintKind'] ?? null;
    out.buildingHeight = null;
    out.synthesized = Boolean(p['r1:synthesized']);
  }
  return out;
}
