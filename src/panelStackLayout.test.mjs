...
test('expanded left panels integrate their headers with the container shell', () => {
  ...
  assert.match(
    css,
    /#left-panel-stack > \[data-panel-id\]:not\(\.collapsed\) \.panel-divider\s*\{[\s\S]*?background:\s*var\(--glass-border-hover\);/,
  );
});
...
test('expanded right panels highlight the title divider without changing collapsed launchers', () => {
  ...
  assert.match(
    css,
    /#right-context-rail \[data-panel-id\]:not\(\.collapsed\) \.panel-divider\s*\{[\s\S]*?background:\s*var\(--glass-border-hover\);/,
  );
  assert.match(
    css,
    /#param-slider-panel:not\(\.collapsed\) \.param-panel-divider\s*\{[\s\S]*?background:\s*var\(--glass-border-hover\);/,
  );
});
