...
const WEATHER_ENABLED_STORAGE_KEY = 'r1.cockpitWeatherEffects.enabled';
...
export class CockpitCloudEffectsController {
  constructor(viewer) {
    ...
    window.addEventListener('r1:cockpit-mode-changed', this.onCockpitMode);
    window.addEventListener('r1:cockpit-weather-toggle', this.onEnabledChange);
  ...
  emitEnabledState() {
    window.dispatchEvent(new CustomEvent('r1:cockpit-weather-state', {
  ...
  destroy() {
    ...
    window.removeEventListener('r1:cockpit-mode-changed', this.onCockpitMode);
    window.removeEventListener('r1:cockpit-weather-toggle', this.onEnabledChange);
